...
- **A namespace-prefixed `<opf:package>` with no version is diagnosed rather than let through.** Reading the package element through `extractElementAttributes` taught the version read to accept an `opf:` prefix, but the fallback beneath it still tested `/<package[\s>]/`. A prefixed root carrying no `version` therefore read as `unreadable` — indistinguishable from a document holding no package element at all — so `resolvePublicationVersion` let it through instead of reporting `OPF-001` and stopping. Both forms now reach the same verdict. Case stays significant: `extractElementAttributes` matches case-insensitively, but XML element names are not, and `<PACKAGE>` is better served falling through for the pipeline to diagnose. Latent rather than a regression — before the refactor neither half accepted a prefix, so both prefixed forms read as unreadable. No fixture in either corpus carries one; parity is unchanged across 763 fixtures.

### Changed

- **The package element is read through the file's own attribute extractor.** `extractElementAttributes` already matched an opening tag with a single capture group and handed the text to `parseAttributes` — the shape the backtracking fix below had just rebuilt by hand, three lines above the `spine` call that used it. Reusing it drops three bespoke patterns and leaves attribute order handled in one place. It also accepts an `opf:` prefix and whitespace around `=`, neither of which the hand-rolled patterns matched; no fixture in either corpus carries those shapes, and parity is unchanged across 763 fixtures.

### Security

A crafted EPUB could stall validation before any rule ran. Five families of scan — reached before, or without, any well-formedness gate — were superlinear in the size of their input, and are now bounded; a sixth defect let archive-controlled entry names resolve up `Object.prototype`. Each regex replacement was checked for equivalence against the pattern it replaces, over hand-written edge cases and randomized inputs, and then across the corpus: **0 regressed, 0 improved on all 763 fixtures.**

- **The package-element attribute match no longer backtracks.** The two order-specific patterns each chained three `[^>]*` sections, which multiply: on a `<package` tag that never closes, 2.6 KB took 105 ms and grew faster than the square of the input, stalling the parse before any validation ran. Matching the element once and reading `version` and `unique-identifier` off the captured attribute text makes attribute order stop mattering — which is what the second pattern existed for — so both collapse into one. 55 KB of unclosed tag now parses in 0.09 ms.
- **Every package-element scan is bounded by `indexOf` rather than `[^>]*`.** Removing the backtracking above fixed one long unclosed tag but left the other shape untouched: `[^>]*` restarts a scan to end of input from every position the tag name matches, so *many short* unclosed tags stayed quadratic. A 3 KB EPUB carrying a 1 MB package document took 95 s, against 0.32 s for Java on the same file. `[^>]*` can never cross a `>`, so the first `>` after the tag name is the only end it could settle on; finding it directly removes the class rather than one instance of it — the same shape was in `nsRegex`, `dirRegex`, `parsePrefixes` and the collection tokenizer. 95 s → 1.26 s, and flat from 1 MB to 4.3 MB where it was quadratic; the collection tokenizer goes 23.6 s → 3 ms on a 700 KB run of unterminated tags.
- **XML comment bounds are found with `indexOf` instead of a lazy scan.** `<!--[\s\S]*?-->` restarts at every `<!--` and runs to the end of the input whenever one never closes, so a 293 KB `container.xml` of repeated `<!--` cost 4.0 s in that regex alone; it is now 1.1 ms. `stripXmlComments` and `blankXmlComments` shared the regex, so this covers all eleven call sites across `checker`, `ocf/container`, `opf/parser` and `opf/validator`. Searching from `start + 4` keeps the regex's edge cases intact — `[\s\S]*?` may match nothing, so `<!-->` and `<!--->` remain non-comments while `<!---->` stays a comment — verified byte-identical over 19 edge cases and 200,000 random inputs.
- **The scans with no XML parse gate in front of them are bounded.** `container.xml`, `encryption.xml` and CSS are read before — or without — any libxml2 well-formedness check, so their scans are the ones a crafted file actually reaches: five element scans in `ocf/container`, the `<EncryptedData>` block scan, CSS comment stripping, and remote-URL detection. Each lazy group ran to the end of the input from every opener that never closed. A 1.3 MB `container.xml` in a 3.6 KB EPUB went from 21.06 s to 0.11 s, with the doubling curve (1.41 s, 5.34 s, 21.06 s) flattening to constant. Equivalence checked per site over 62 edge cases and 670,000 random inputs.
- **The CSS `url()` scan is bounded by the characters its body cannot cross.** The previous fix claimed this scan; it did not have it. Bounding each candidate at the first `)` was useless, because `[^"')]+` accepts `u`, `r`, `l`, `(`, `/` and `:` — so on `url(https://a` repeated, every candidate still ran to the end of the stylesheet (508 KB: 15,602 ms before, 15,211 ms after — no regression, but no fix either). The body cannot cross `"`, `'` or `)`, and giving those back never helps, since an earlier `)` would itself be one of them; one cursor over all three decides every candidate in constant time. A 254 KB payload in an 1858-byte EPUB drops from 4.37 s to 0.12 s, and 2 MB now costs 0.15 s. Benign stylesheets get faster too — 977 KB in 0.008 ms against the old 0.139 ms. Equivalence checked over 40 edge cases and 750,000 random inputs across three alphabets.
- **ZIP entries are keyed by `Map`, so prototype names can no longer answer lookups.** Entry names come from the archive, so they reach the lookup as arbitrary strings; backed by a plain object they resolved up the prototype chain — `has` answered true for files no EPUB contained, and the read handed back a function where bytes were expected, which threw out of `strFromU8` as a raw `TypeError`. A `<rootfile full-path="constructor">` reached all three, turning a missing package document into a FATAL quoting a Node internal message; it now reports "Package document not found", matching Java's `OPF-002`. A `Map` also matches `ValidationContext.files`, which already is one. The reciprocal write side stays broken upstream and is recorded in `PROJECT_STATUS.md`: fflate's `unzipSync` assigns entries onto a plain object, so an entry named `__proto__` is lost before this code sees it.
