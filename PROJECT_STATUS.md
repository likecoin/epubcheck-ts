...
**Overall: ~97% complete (1333 tests passing, 34 skipped, 1367 total)**
...
├── epub2.integration.test.ts         #  99 tests  ( 88 pass,  11 skip) - EPUB 2 (all 7 Java features)
└── profiles.integration.test.ts      # 125 tests  (115 pass,  10 skip) - 9 profile extensions
```
...
**Critical gaps:**
- 🟢 **EPUB 2 validation** - 89% of 99 Java scenarios pass (88/99). Remaining gaps: EPUB 2-specific OPF Schematron rules, metadata validation (OPF-052/054/055), NCX content checks (NCX-006, RSC-010 for NCX), XHTML 1.1 DOCTYPE/HTML5 rejection
- 🟢 **Profile implementation** - 92% of 125 profile scenarios pass (115/125). Remaining gaps: collection metadata validation edge cases, sub-collection hierarchy rules, EDUPUB accessibility requirements, multiple-rendition metadata.xml
...
**Integration tests (32)** - Grouped by category:
...
- **EPUB 2 (11 skipped)**: Remaining EPUB 2-specific OPF Schematron rules, metadata validation (OPF-052/054/055), NCX content checks (NCX-006, RSC-010 for NCX), XHTML 1.1 DOCTYPE/HTML5 rejection. Most are EPUB 2-specific rules our EPUB 3-focused validator doesn't implement.
- **Profiles (10 skipped)**: Remaining profile-specific validation gaps. Each skipped test annotates the specific missing check (collection metadata validation edge cases, sub-collection hierarchy rules, multi-rendition metadata.xml, etc.). See the Implementation Gaps Backlog section below for the full breakdown.
...
10. **EPUB 2 coverage** - Validator has broad EPUB 2 support. 99 EPUB 2 scenarios are ported with 11 skipped — remaining gaps are EPUB 2-specific OPF Schematron, metadata, and NCX content checks.
...
| Java Feature | Scenarios | Active | Skipped | Pass Rate |
|---|---:|---:|---:|---:|
| **EPUB 2 (all 7 features)** | **99** | **88** | **11** | **89%** |
...
The 32 skipped integration tests form a prioritized backlog. For the authoritative per-test breakdown, search `it.skip` in `test/` — each skip has an inline annotation of the specific validator gap. Themes by category:
...
### EPUB 2-specific checks (11 tests)
